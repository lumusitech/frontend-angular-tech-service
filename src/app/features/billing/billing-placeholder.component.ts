  ...
  template: `
    <div class="space-y-4">
      <div>
        ...
        <p class="text-gray-500 dark:text-gray-400 mt-1">{{ 'billing.subtitle' | translate }}</p>
      </div>
      <div class="flex flex-col items-center justify-center py-16 px-4">
        <div
          class="w-16 h-16 bg-blue-100 dark:bg-blue-900/30 rounded-full flex items-center justify-center mb-4"
        >
          <mat-icon class="text-blue-600 dark:text-blue-400 text-3xl">receipt_long</mat-icon>
        </div>
        <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-1">
          {{ 'billing.comingSoon' | translate }}
        </h3>
        <p class="text-sm text-gray-500 dark:text-gray-400 text-center max-w-sm">
          {{ 'billing.comingSoonMessage' | translate }}
        </p>
      </div>
    </div>
  `,
