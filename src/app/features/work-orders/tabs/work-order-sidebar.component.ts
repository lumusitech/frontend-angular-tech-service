...
@Component({
  ...
  template: `
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4">
      <div
        class="bg-white dark:bg-gray-800/90 rounded-2xl border border-gray-200 dark:border-gray-700/70 p-4 shadow-xs backdrop-blur-sm"
      >
        <div class="flex items-center justify-between mb-3">
          ...
          <button
            mat-button
            color="primary"
            (click)="editTechnicians.emit()"
            class="px-2! py-1! min-h-8! text-xs font-medium"
          >
          ...
          <div class="space-y-2">
            @for (tech of technicians(); track tech.id) {
              <div class="flex items-center gap-2.5 p-2 rounded-xl bg-gray-50 dark:bg-gray-700/40">
                <div
                  class="w-7 h-7 bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400 rounded-full flex items-center justify-center font-bold text-xs shrink-0"
                >
                  {{ tech.name.charAt(0) }}
                </div>
      ...
      >
        ...
        <div class="space-y-2.5 text-xs sm:text-sm">
          <div
            class="flex justify-between items-center py-1 border-b border-gray-100 dark:border-gray-700/40"
          >
