        ...
        <div class="relative isolate">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8 relative z-10">
            @for (step of steps; track step.number) {
              <div
                class="group flex flex-col items-center text-center relative bg-gray-50/70 dark:bg-gray-900/50 p-6 sm:p-5 rounded-2xl border border-gray-200/60 dark:border-gray-800 hover:border-blue-300 dark:hover:border-blue-700 transition-all duration-300"
              >
                ...
                >
                  <span class="text-xs font-bold text-gray-400 dark:text-gray-500 leading-none"
                    >0{{ step.number }}</span
                  >
                  <mat-icon class="!w-5 !h-5 !text-xl text-blue-600 dark:text-blue-400 mt-0.5">{{
                    step.icon
                  }}</mat-icon>
                </div>
