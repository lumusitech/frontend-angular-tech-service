  ...
  search(query: string): void {
    ...
    forkJoin({
      users: this.usersService
        .getAll({ search: query, limit: 3 })
        .pipe(catchError(() => of({ data: [], total: 0, page: 1, limit: 3, totalPages: 0 }))),
      clients: this.clientsService
        .getAll({ search: query, limit: 3 })
        .pipe(catchError(() => of({ data: [], total: 0, page: 1, limit: 3, totalPages: 0 }))),
      workOrders: this.workOrdersService
        .getAll({ search: query, limit: 3 })
        .pipe(catchError(() => of({ data: [], total: 0, page: 1, limit: 3, totalPages: 0 }))),
      ...
      skills: this.skillsService
        .getAll({ search: query, limit: 3 })
        .pipe(catchError(() => of({ data: [], total: 0, page: 1, limit: 3, totalPages: 0 }))),
      inquiries: this.inquiriesService
        .getAll({ search: query, limit: 3 })
        .pipe(catchError(() => of({ data: [], total: 0, page: 1, limit: 3, totalPages: 0 }))),
      expenses: this.expensesService
        .getAll({ search: query, limit: 3 })
        .pipe(catchError(() => of({ data: [], total: 0, page: 1, limit: 3, totalPages: 0 }))),
      pendingItems: this.pendingItemsService
        .getAll({ search: query, limit: 3 })
        .pipe(catchError(() => of({ data: [], total: 0, page: 1, limit: 3, totalPages: 0 }))),
      ...
      ({
        ...
        const results: SearchResult[] = [
          ...users.data.map((u: User) => ({
            ...
            icon:
              u.role === 'technician' ? 'engineering' : u.role === 'seller' ? 'badge' : 'person',
            route: `/admin/users`,
          })),
